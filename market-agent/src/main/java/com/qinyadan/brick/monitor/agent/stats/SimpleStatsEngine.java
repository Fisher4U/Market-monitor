...
public class SimpleStatsEngine {
	...
	public void mergeStats(SimpleStatsEngine other) {
		for (Map.Entry<String, StatsBase> entry : other.stats.entrySet()) {
			StatsBase ourStats = (StatsBase) this.stats.get(entry.getKey());
			StatsBase otherStats = (StatsBase) entry.getValue();
			if (ourStats == null) {
	...
	public List<MetricData> getMetricData(Normalizer metricNormalizer, MetricIdRegistry metricIdRegistry,
			String scope) {
		List<MetricData> result = new ArrayList(this.stats.size() + 1);
		boolean isTrimStats = false;//ServiceFactory.getConfigService().getDefaultAgentConfig().isTrimStats();
		...
		for (Map.Entry<String, StatsBase> entry : this.stats.entrySet()) {
			...
			MetricData metricData = createMetricData(metricName, (StatsBase) entry.getValue(), metricNormalizer,
					metricIdRegistry);
			if (metricData != null) {
	...
	protected static MetricData createMetricData(MetricName metricName, StatsBase statsBase,
		...
		String normalized = metricNormalizer.normalize(metricName.getName(), Form.NFC);
		if (normalized == null) {
			return null;
		}
