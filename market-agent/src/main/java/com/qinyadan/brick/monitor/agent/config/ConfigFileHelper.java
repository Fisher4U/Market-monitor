...
public class ConfigFileHelper {
	...
	private static File findFromProperty() {
		...
		if (filePath != null) {
			...
			System.err.println(
					MessageFormat.format("The configuration file {0} specified with the {1} property does not exist",
							new Object[] { configFile

									.getAbsolutePath(), "newrelic.config.file" }));
		}
		return null;
	}
	...
	private static File findHomeDirectoryFromEnvironmentVariable() {
		...
		if (filePath != null) {
			...
			System.err.println(
					MessageFormat.format("The directory {0} specified with the {1} environment variable does not exist",
							new Object[] { homeDir

									.getAbsolutePath(), "NEWRELIC_HOME" }));
		}
		return null;
	}
