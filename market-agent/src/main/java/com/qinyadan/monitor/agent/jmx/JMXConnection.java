...
import com.qinyadan.monitor.utils.StringUtils;
...
public class JMXConnection {
	...
	public synchronized List<JMXConnectionInfo> getMBeanConnection() {
		...
		List<JMXConnectionInfo> connections = connectCacheLibrary.entrySet().stream()
				.filter(entry -> NumberUtils.isNumber(entry.getKey().replace(serverName, ""))).map(Map.Entry::getValue)
	...
	private void clearCache() {
		// 清除当前连接池中的连接
		List<String> removeKey = connectCacheLibrary.keySet().stream()
				.filter(key -> NumberUtils.isNumber(key.replace(serverName, ""))).collect(Collectors.toList());
	...
	private JMXConnectUrlInfo getConnectorAddress(VirtualMachineDescriptor desc) {
		...
		try {
			jmxConnectUrlInfo = AbstractJmxCommand.findJMXRemoteUrlByProcessId(Integer.parseInt(desc.id()),
	 ...
	 * @throws IOException
	 */
	private JMXConnectionInfo initJMXConnectionInfo(JMXConnector connector, VirtualMachineDescriptor desc)
			throws IOException {
		JMXConnectionInfo jmxConnectionInfo = new JMXConnectionInfo();
