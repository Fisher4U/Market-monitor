...
import com.qinyadan.monitor.agent.utils.CommandUtilForUnix;
...
public class AbstractJmxCommand {
	 ...
	 * @param ip
	 *            应用所在的IP地址
	 * @return 返回查找的JMX连接地址对象或查找失败返回Null
	 */
		...
		try {
			...
			if (result.isSuccess) {
				...
				if (isAuth) {
					...
					String contentForAccess = CommandUtilForUnix
							.execWithReadTimeLimit(String.format("cat %s", accessFile), false, 7).msg;
					String user = getJmxUser(contentForAccess);
					String contentForPassword = CommandUtilForUnix
							.execWithReadTimeLimit(String.format("cat %s", passwordFile), false, 7).msg;
	...
	/**
	 * 获取JMX授权用户 JMX 用户经过测试,必须是readwrite权限的
	 * 
	 * @param content
	 * @return
	 */
	 ...
	 * @param pid
	 *            查找的进行id
	 * @return 返回查找的JMX本地连接地址或查找失败返回Null
	 */
