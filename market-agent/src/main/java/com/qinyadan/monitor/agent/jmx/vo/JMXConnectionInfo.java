...
public class JMXConnectionInfo {
	...
	public String toString() {
		return "JMXConnectionInfo{" + "cacheKeyId='" + cacheKeyId + '\'' + ", mBeanServerConnection="
				+ mBeanServerConnection + ", jmxConnector=" + jmxConnector + ", connectionQualifiedServerName='"
				+ connectionQualifiedServerName + '\'' + ", connectionServerName='" + connectionServerName + '\''
				+ ", name='" + name + '\'' + ", pid=" + pid + ", valid=" + valid + '}';
	}
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof JMXConnectionInfo))
			return false;

		JMXConnectionInfo that = (JMXConnectionInfo) o;

		return new EqualsBuilder().append(pid, that.pid).append(valid, that.valid).append(cacheKeyId, that.cacheKeyId)
				...
				.append(connectionServerName, that.connectionServerName).append(name, that.name).isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(cacheKeyId).append(mBeanServerConnection)
				.append(connectionQualifiedServerName).append(connectionServerName).append(name).append(pid)
				.append(valid).toHashCode();
	}
	...
	/**
	 * 注：请勿使用此对象进行JMX连接交互操作，可能会在JMX维护时发生java.io.IOException: The client has been
	 * closed异常 （{@link JMXManager#getJmxMetricValue(java.lang.String)} 方法除外）
	 * 
	 * @return
	 */
